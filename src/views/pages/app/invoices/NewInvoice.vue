    ...
    <div class="invoice overflow-auto" ref="topclass">
        <div style="min-width: 600px">
            ...
            <main>
                ...
                <h3 class="text-center capitalize mb-4">{{ invoice.invoice.name.toUpperCase() }}</h3>
                ...
                <h5 class="mb ml" v-if="invoice.invoice.invoice_type === 'proforma invoice'">{{ information }}</h5>
                <h5 class="mb ml" v-if="invoice.invoice.invoice_type === 'quotation'">{{ quotationNote }}</h5>

                <table cellpadding="0" cellspacing="0" class="more-info" v-if="invoice.invoice.invoice_type === 'proforma invoice' || invoice.invoice.invoice_type === 'quotation'">
                    <tr v-if="invoice.invoice.country_of_origin">
                        <th>Country of Origin</th>
                        <td class="bold">{{ invoice.invoice.country_of_origin }}</td>
                    </tr>
                    
                    <tr v-if="invoice.invoice.condition_of_sale" data-iterate="tax">
                        <th>Condition of Sale:</th>
                        <td class="bold">{{ invoice.invoice.condition_of_sale }}</td>
                    </tr>
                    
                    <tr v-if="invoice.invoice.terms_of_payment">
                        <th>Terms of Payment:</th>
                        <td class="bold">{{ invoice.invoice.terms_of_payment }}</td>
                    </tr>

                    <tr v-if="invoice.invoice.delivery" class="amount-total">
                        <th>Delivery</th>
                    ...
                    <tr v-if="invoice.invoice.validity" data-hide-on-quote="true">
                        <th>Validity:</th>
                        <td class="bold">{{ invoice.invoice.validity }}</td>
                    </tr>

                    <tr v-if="invoice.invoice.installation" data-hide-on-quote="true">
                        <th>Installation:</th>
                        <td class="bold">{{ invoice.invoice.installation }}</td>
                    </tr>
                    <tr v-if="invoice.invoice.place_of_delivery" data-hide-on-quote="true">
                        <th>Place of Delivery:</th>
                        <td class="bold">{{ invoice.invoice.place_of_delivery }}</td>
                    </tr>
                    <tr v-if="invoice.invoice.invoice_type === 'proforma invoice'">
                        <th>Note:</th>
                ...
                <div class="notices" v-if="invoice.invoice.invoice_type === 'proforma invoice' || invoice.invoice.invoice_type === 'quotation'">
...
export default {
    data() {
        return {
            ...
            quotationNote: 'NOTE: In the process of repairs, any additional part found faulty or not suitable to be refixed will attract extra cost but subject to your verification and approval.'
        }
    },
