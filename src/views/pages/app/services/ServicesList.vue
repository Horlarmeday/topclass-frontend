<!-- =========================================================================================
  File Name: ServicesList.vue
  Description: Service List - List View
========================================================================================== -->

<template>
  <div id="data-list-list-view" class="data-list-container">

    <edit-service :isSidebarActive="addNewDataSidebar" @closeSidebar="toggleDataSidebar" :data="sidebarData" />

    <export-service :displayExport="displayExport" @hideDisplay="hideDisplayExport" v-if="displayExport"/>

    <vs-table :sst="true" @search="handleSearch" ref="table" multiple v-model="selected" :max-items="itemsPerPage" search :data="services" :total="queriedItems">
      ...
      <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
        ...
        <vs-dropdown vs-trigger-click class="cursor-pointer mb-4 mr-4 items-per-page-handler">
          <div class="p-4 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
            <span class="mr-2">{{ currentPage * itemsPerPage - (itemsPerPage - 1) }} - {{ services.length - currentPage * itemsPerPage > 0 ? currentPage * itemsPerPage : services.length }} of {{ queriedItems }}</span>
            <feather-icon icon="ChevronDownIcon" svgClasses="h-4 w-4" />
          </div>
          ...
          <tbody>
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td class="whitespace-no-wrap">
                  ...
                  <feather-icon icon="EyeIcon" svgClasses="w-5 h-5 hover:text-danger stroke-current" class="ml-2" @click="viewData(tr.svid)" />
              </vs-td>
            </vs-tr>
          </tbody>
        </template>
...
export default {
  ...
  computed: {
    ...
    pages() {
      return this.$store.state.service.pages
    }
  },
  methods: {
    handleError(error) {
    ...
    deleteData (svid) {
      this.$store.dispatch('service/removeService', svid).then(response => this.handleSuccess(response)).catch(err => { this.handleError(err) })
    },
    editData (data) {
    ...
    viewData(id) {
      this.$router.push(`/app/service/${id}`)
    },

    showDisplayPrompt (service) {
    ...
    handleSearch(search) {
      this.currentPage = 1
      this.$store.dispatch('service/fetchServices', { currentPage: this.currentPage, itemsPerPage: this.itemsPerPage, search })
    },

    handlePageChange() {
      this.$store.dispatch('service/fetchServices', { currentPage: this.currentPage, itemsPerPage: this.itemsPerPage })
    },

    filterPage(value) {
      this.$store.dispatch('service/fetchServices', { currentPage: this.currentPage, itemsPerPage: this.itemsPerPage, filter: value })
    },
