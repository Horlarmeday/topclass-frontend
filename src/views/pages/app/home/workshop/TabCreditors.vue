...
export default {
    ...
    methods: {
        handleChangePage(page) {
            axios.get('/sales/staff-creditors', { params: { currentPage: page, pageLimit: this.itemsPerPage } })
            .then(response => { 
    ...
    created() {
        axios.get('/sales/staff-creditors', { params: { currentPage: 1, pageLimit: this.itemsPerPage } })
        .then(response => { 
            this.creditors = response.data.data.docs
            this.pages = response.data.data.total
        }).catch(error => this.handleError(error))
